...
import * as TAGS from '../resources/vhtml-tags.json';
import * as ATTRS from '../resources/vhtml-attributes.json';
...
const provider = {
  ...
  getTagMethods(tag) {
      return (TAGS[tag] && TAGS[tag].methods) || [];
  },
  ...
  getAttrSuggestion({editor, bufferPosition, prefix}) {
    ...
    const tagMethods = this.getTagMethods(tag);
    ...
    useAttrs = method ? tagMethods : tagAttrs;


    useAttrs.forEach(attr => {
        const sug = this.buildAttrSuggestion({attr, tag, null, null}, { });
  ...
  buildAttrValueSuggestion(tag, attr, value) {;
    ...
    return {
      ...
      rightLabel: attrItem.global ? 'vhtml-ui' : `<${tag}>`
    };
  },
